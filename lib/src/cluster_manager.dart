...
class ClusterManager<T extends cm.ClusterItem> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<Marker> Function(cm.Cluster<T>)? markerBuilder,
  ...
  final Future<Marker> Function(cm.Cluster<T>) markerBuilder;
  ...
  void _updateClusters() async {
    List<cm.Cluster<T>> mapMarkers = await getMarkers();
  ...
  /// Add on cluster item
  void addItem(cm.ClusterItem newItem) {
  ...
  Future<List<cm.Cluster<T>>> getMarkers() async {
    ...
    if (stopClusteringZoom != null && _zoom >= stopClusteringZoom!)
      return visibleItems.map((i) => cm.Cluster<T>.fromItems([i])).toList();

    List<cm.Cluster<T>> markers;
  ...
  List<cm.Cluster<T>> _computeClustersWithMaxDist(
  ...
  List<cm.Cluster<T>> _computeClusters(
      List<T> inputItems, List<cm.Cluster<T>> markerItems,
      {int level = 5}) {
    ...
    markerItems.add(cm.Cluster<T>.fromItems(items));
  ...
  static Future<Marker> Function(cm.Cluster) get _basicMarkerBuilder =>
      (cluster) async {
        return Marker(
